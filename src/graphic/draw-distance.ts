...
const getDistanceForGraphicsLevel = (level: GraphicLevels): number => {
  ...
  if (!WorldDistances[level]) {
    console.warn(`Unknown graphics level ${level}, defaulting to medium`)
    return WorldDistances[GraphicLevels.Medium]
  }

  const distance = WorldDistances[level]
...
export class DrawDistance extends TypedEventTarget<{ 'distance-changed': number }> {
  constructor(graphics: GraphicEngine) {
    super()
    this._distance = getDistanceForGraphicsLevel(graphics.getSettings().level)
    graphics.addEventListener('settingsChanged', (event) => {
      this.distance = getDistanceForGraphicsLevel(event.detail.level)
    })
  }
